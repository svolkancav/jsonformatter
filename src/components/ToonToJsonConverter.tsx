...
interface ToonToJsonConverterProps {
  onJsonGenerated?: (json: string) => void;
}

export function ToonToJsonConverter({ onJsonGenerated }: ToonToJsonConverterProps) {
  ...
  const validateAndConvert = useCallback(() => {
    ...
    try {
      if (!toonInput.trim()) {
        setError('Please enter TOON data to convert');
        setLoading(false);
        return;
      }

      // Parse TOON to object
      const parsedData = decode(toonInput);
      ...
      setError(`Invalid TOON: ${err instanceof Error ? err.message : 'Unknown error'}`);
      setLoading(false);
    }
  }, [toonInput, prettyPrint, onJsonGenerated]);
    ...
    try {
      ...
      setTimeout(() => {
        try {
          ...
          setError(`TOON parsing error: ${parseErr instanceof Error ? parseErr.message : 'Unknown error'}`);
          setLoading(false);
        }
      }, 100);
    } catch (err) {
  ...
  2,Bob,user'
      ...
      <button
        ...
        disabled={loading || !toonInput.trim()}
